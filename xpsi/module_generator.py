...
for instrument in args.instrument:
    module += (
    '''
parser.add_argument('--{0}-data-folder', 
                    type=str, default=None, comment=True, comment_line_above='{0}',
                    help='{1} {0} path to the data directory to add in front of all subsequent files. If not given, will be considered empty and use the full path for all the data.')
...
parser.add_argument('--{0}-input-bounds', type=int, nargs=2,
...
parser.add_argument('--{0}-energy-independent-effective-area-scaling-factor-value', 
                    type=str, action=CompileAction, comment=True, empty_lines_below=2,
...
parser.add_argument('--{0}-energy-independent-effective-area-scaling-factor-bounds', type=str, nargs=2, 
                    action=CompileAction, comment=True,
...
parser.add_argument('--{0}-energy-independent-effective-area-scaling-factor-prior',
                    type=str, nargs='*', default=['truncnorm.ppf(x, -3.0, 3.0, loc=1.0, scale=0.1)'], action=NullAction, comment=True,
...
parser.add_argument('--{0}-phase-shift-bounds',
                    type=str, nargs=2, action=CompileAction,
...
parser.add_argument('--{0}-phase-shift-value',
                    type=str, action=CompileAction, comment=True,
                    ...
                    empty_lines_below=2)
parser.add_argument('--{0}-background-path', type=str, help='{1} {0} background spectrum file.',
                    comment=True)
parser.add_argument('--{0}-background-prior-support-half-width', type=float, help='{0} background prior support half-width. The half-width is in units of standard deviation of background count number per instrument channel.',
                    comment=True, empty_lines_below=3)
...
{0}.data = CustomData.load(args.{0}_counts_file,
